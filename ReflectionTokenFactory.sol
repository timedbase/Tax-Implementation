...
/**
 * @title ReflectionTokenFactory
 * @notice Factory for deploying ReflectionToken clones using EIP-1167 minimal proxy pattern
 */
contract ReflectionTokenFactory {
    ...
    uint256 public deploymentFee = 0.00055 ether;  // 2.5x Tax Token fee
    ...
    constructor(address _feeRecipient) {
        _owner = msg.sender;
        implementation = address(new ReflectionToken());
        feeRecipient = _feeRecipient == address(0) ? msg.sender : _feeRecipient;
        emit OwnershipTransferred(address(0), msg.sender);
    }

    /**
     * @notice Create a new reflection token with default settings
    ...
    function createToken(
        ...
        uint256 swapThreshold = totalSupply / 5000;  // 0.02%

        // Caller is owner and all fee recipients, all taxes start at 0
        ReflectionToken(payable(tokenAddress)).initialize(
            name,
            symbol,
            totalSupply,
            [msg.sender, msg.sender],  // marketing, team = caller
            defaultRouter,
            [uint256(0), 0, 0, 0, 0],  // buy taxes all 0 (marketing, team, lp, burn, reflection)
    ...
    /**
     * @notice Create a new reflection token with custom taxes
     * @param name Token name
     * @param symbol Token symbol
     * @param totalSupply Total supply (with 18 decimals)
     * @param wallets [marketing, team] wallet addresses
     * @param buyTaxes [marketing, team, liquidity, burn, reflection] in basis points
     * @param sellTaxes [marketing, team, liquidity, burn, reflection] in basis points
     */
    function createTokenWithTaxes(
        string calldata name,
        string calldata symbol,
        uint256 totalSupply,
        address[2] calldata wallets,
        uint256[5] calldata buyTaxes,
        uint256[5] calldata sellTaxes
    ) external payable returns (address tokenAddress) {
        require(msg.value >= deploymentFee, "Insufficient fee");
        require(bytes(name).length > 0 && bytes(symbol).length > 0, "Empty name/symbol");
        require(totalSupply > 0, "Zero supply");
        require(wallets[0] != address(0) && wallets[1] != address(0), "Zero wallet");
