...
import { REFLECTION_TOKEN_ABI } from '@/config/contracts';
import { formatUnits, parseUnits, isAddress } from 'viem';

interface ReflectionTokenManagementProps {
  tokenAddress: `0x${string}`;
  onClose: () => void;
}

export function ReflectionTokenManagement({ tokenAddress, onClose }: ReflectionTokenManagementProps) {
  ...
  const { data: tokenData, refetch } = useReadContracts({
    contracts: [
      { address: tokenAddress, abi: REFLECTION_TOKEN_ABI, functionName: 'name' },
      { address: tokenAddress, abi: REFLECTION_TOKEN_ABI, functionName: 'symbol' },
      { address: tokenAddress, abi: REFLECTION_TOKEN_ABI, functionName: 'owner' },
      { address: tokenAddress, abi: REFLECTION_TOKEN_ABI, functionName: 'totalSupply' },
      // Buy taxes
      { address: tokenAddress, abi: REFLECTION_TOKEN_ABI, functionName: 'buyMarketingTax' },
      { address: tokenAddress, abi: REFLECTION_TOKEN_ABI, functionName: 'buyTeamTax' },
      { address: tokenAddress, abi: REFLECTION_TOKEN_ABI, functionName: 'buyLiquidityTax' },
      { address: tokenAddress, abi: REFLECTION_TOKEN_ABI, functionName: 'buyBurnTax' },
      { address: tokenAddress, abi: REFLECTION_TOKEN_ABI, functionName: 'buyReflectionTax' },
      // Sell taxes
      { address: tokenAddress, abi: REFLECTION_TOKEN_ABI, functionName: 'sellMarketingTax' },
      { address: tokenAddress, abi: REFLECTION_TOKEN_ABI, functionName: 'sellTeamTax' },
      { address: tokenAddress, abi: REFLECTION_TOKEN_ABI, functionName: 'sellLiquidityTax' },
      { address: tokenAddress, abi: REFLECTION_TOKEN_ABI, functionName: 'sellBurnTax' },
      { address: tokenAddress, abi: REFLECTION_TOKEN_ABI, functionName: 'sellReflectionTax' },
      // Wallets
      { address: tokenAddress, abi: REFLECTION_TOKEN_ABI, functionName: 'marketingWallet' },
      { address: tokenAddress, abi: REFLECTION_TOKEN_ABI, functionName: 'teamWallet' },
      // Settings
      { address: tokenAddress, abi: REFLECTION_TOKEN_ABI, functionName: 'swapThreshold' },
      { address: tokenAddress, abi: REFLECTION_TOKEN_ABI, functionName: 'swapEnabled' },
      { address: tokenAddress, abi: REFLECTION_TOKEN_ABI, functionName: 'pancakePair' },
      { address: tokenAddress, abi: REFLECTION_TOKEN_ABI, functionName: 'totalFees' },
    ],
  });
  ...
  const buyLiquidityTax = Number(tokenData?.[6]?.result || 0);
  const buyBurnTax = Number(tokenData?.[7]?.result || 0);
  const buyReflectionTax = Number(tokenData?.[8]?.result || 0);

  const sellMarketingTax = Number(tokenData?.[9]?.result || 0);
  const sellTeamTax = Number(tokenData?.[10]?.result || 0);
  const sellLiquidityTax = Number(tokenData?.[11]?.result || 0);
  const sellBurnTax = Number(tokenData?.[12]?.result || 0);
  const sellReflectionTax = Number(tokenData?.[13]?.result || 0);

  const marketingWallet = tokenData?.[14]?.result as string | undefined;
  const teamWallet = tokenData?.[15]?.result as string | undefined;

  const swapThreshold = tokenData?.[16]?.result as bigint | undefined;
  const swapEnabled = tokenData?.[17]?.result as boolean | undefined;
  const pancakePair = tokenData?.[18]?.result as string | undefined;
  const totalFees = tokenData?.[19]?.result as bigint | undefined;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-3 overflow-y-auto">
      <div className="bg-[#0a0a0a] border border-[#1a1a1a] rounded-lg p-3 max-w-2xl w-full my-3 max-h-[92vh] overflow-y-auto">
        ...
        <div className="flex items-start justify-between mb-2.5">
          <div className="min-w-0 flex-1 mr-2">
            <div className="flex items-center gap-2 mb-0.5">
              <h2 className="text-sm font-semibold text-balance truncate">{name || 'Loading...'}</h2>
        ...
        {/* Reflection Stats */}
        <div className="bg-purple-500/10 border border-purple-500/20 rounded p-2 mb-2.5">
          <p className="text-[10px] text-purple-400">
            Total Reflected: {totalFees ? Number(formatUnits(totalFees, 18)).toLocaleString() : '0'} tokens
          </p>
        </div>
        ...
        <div>
          {activeTab === 'taxes' && (
            <ReflectionTaxConfiguration
              tokenAddress={tokenAddress}
              buyTaxes={{ marketing: buyMarketingTax, team: buyTeamTax, liquidity: buyLiquidityTax, burn: buyBurnTax, reflection: buyReflectionTax }}
              sellTaxes={{ marketing: sellMarketingTax, team: sellTeamTax, liquidity: sellLiquidityTax, burn: sellBurnTax, reflection: sellReflectionTax }}
...
// Tax Configuration Component for Reflection Token
function ReflectionTaxConfiguration({ tokenAddress, buyTaxes, sellTaxes, writeContract, isPending, isConfirming, isSuccess, error }: any) {
  const [buyForm, setBuyForm] = useState({
    marketing: buyTaxes.marketing / 100,
    team: buyTaxes.team / 100,
    liquidity: buyTaxes.liquidity / 100,
    burn: buyTaxes.burn / 100,
    reflection: buyTaxes.reflection / 100,
  });
  ...
  const [sellForm, setSellForm] = useState({
    ...
    reflection: sellTaxes.reflection / 100,
  });
  ...
  const handleUpdateSellTaxes = () => {
    ...
    writeContract({
      ...
      args: [
        ...
        sellForm.burn * 100,
        sellForm.reflection * 100,
      ],
    });
  };

  const taxFields = ['marketing', 'team', 'liquidity', 'burn', 'reflection'];
          ...
          {taxFields.map((key) => (
            <div key={key}>
              <label htmlFor={`buy-${key}`} className="block text-[9px] text-[#888] mb-0.5 capitalize truncate">
                {key === 'marketing' ? 'Mkt' : key === 'liquidity' ? 'Liq' : key === 'reflection' ? 'Rfl' : key.slice(0, 4)}
              </label>
              <input
                id={`buy-${key}`}
                type="number"
...
// Wallet Management for Reflection Token (only 2 wallets)
function ReflectionWalletManagement({ tokenAddress, marketingWallet, teamWallet, writeContract, isPending, isConfirming, isSuccess, error }: any) {
  const [form, setForm] = useState({
    marketing: marketingWallet || '',
    team: teamWallet || '',
  });

  const allValid = isAddress(form.marketing) && isAddress(form.team);
...
// Settings Component for Reflection Token
function ReflectionSettings({ tokenAddress, swapThreshold, swapEnabled, pancakePair, totalSupply, writeContract, isPending, isConfirming, isSuccess, error }: any) {
...
// Ownership Management Component for Reflection Token
function ReflectionOwnershipManagement({ tokenAddress, currentOwner, writeContract, isPending, isConfirming, isSuccess, error }: any) {
